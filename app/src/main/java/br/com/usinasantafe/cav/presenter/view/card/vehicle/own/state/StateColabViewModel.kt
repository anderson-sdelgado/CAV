...
) : UiStateWithStatus<StateColabState> {

    override fun copyWithStatus(status: UiStatusState): StateColabState =
        copy(status = status)
...
class StateColabViewModel @Inject constructor(
    ...
    init {
        updateState {
            copy(
                option = Option.entries[this@StateColabViewModel.option],
                type = Type.entries[this@StateColabViewModel.type]
            )
        }
    }
