...
interface UpdateTableSupportTeams {
...
class IUpdateTableSupportTeams @Inject constructor(
    ...
    override suspend fun invoke(
        ...
        flowCall(getClassAndMethod()) {
            ...
            val entityList = supportTeamsRepository.listAll(token).getOrThrow()

            emitProgress(count, sizeAll, LevelUpdate.CLEAN, TB_SUPPORT_TEAMS)
            supportTeamsRepository.deleteAll().getOrThrow()

            emitProgress(count, sizeAll, LevelUpdate.SAVE, TB_SUPPORT_TEAMS)
            supportTeamsRepository.addAll(entityList).getOrThrow()
