...
data class StateColabOwnState(
    val option: Option = Option.INSERT,
    val type: Type = Type.MAIN,
    val idSelection: Int = 1,
    override val status: UiStatusState = UiStatusState()
) : UiStateWithStatus<StateColabOwnState> {

    override fun copyWithStatus(status: UiStatusState): StateColabOwnState =
        copy(status = status)

}

@HiltViewModel
class StateColabOwnViewModel @Inject constructor(
    ...
    private val _uiState = MutableStateFlow(StateColabOwnState())
    val uiState = _uiState.asStateFlow()

    private val state get() = uiState.value

    private fun updateState(block: StateColabOwnState.() -> StateColabOwnState) {
    ...
    init {
        updateState {
            copy(
                option = Option.entries[this@StateColabOwnViewModel.option],
                type = Type.entries[this@StateColabOwnViewModel.type]
            )
        }
    }
