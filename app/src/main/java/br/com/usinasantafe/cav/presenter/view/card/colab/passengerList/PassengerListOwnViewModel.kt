package br.com.usinasantafe.cav.presenter.view.card.colab.passengerList
...
data class PassengerListOwnState(
    val option: Option = Option.INSERT,
    val list: List<ItemListScreenModel> = emptyList(),
    val idSelection: Int = 0,
    val flagDialogCheck: Boolean = false,
    override val status: UiStatusState = UiStatusState()
) : UiStateWithStatus<PassengerListOwnState> {

    override fun copyWithStatus(status: UiStatusState): PassengerListOwnState =
        copy(status = status)

}

@HiltViewModel
class PassengerListOwnViewModel @Inject constructor(
    private val listPassenger: ListPassenger,
    private val deletePassenger: DeletePassenger
) : ViewModel() {

    private val _uiState = MutableStateFlow(PassengerListOwnState())
    val uiState = _uiState.asStateFlow()

    private val state get() = uiState.value

    private fun updateState(block: PassengerListOwnState.() -> PassengerListOwnState) {
