...
class IGetPlateTest {
    ...
    @Test
    fun check_get_plate_insert() = runTest {
        ...
        assertEquals(result2.getOrNull(), "ABC-1234")
    }

    @Test
    fun check_get_plate_edit() = runTest {
        val data = CardSharedPreferencesModel(
            vehicleExternalList = listOf(
                VehicleExternalSharedPreferencesModel(
                    id = 1,
                    vehicle = VehicleSharedPreferencesModel(plate = "XYZ-0000")
                )
            )
        )
        ...
        assertEquals(result.isSuccess, true)
        assertEquals(result.getOrNull(), "XYZ-0000")
    }
}
