...
class InvolvedDataViewModelTest {
    ...
    private val viewModel = InvolvedDataViewModel(
        savedStateHandle = SavedStateHandle(
            mapOf(
                Args.FLOW_NOTE_ARG to FlowNote.INVOLVED.ordinal,
    ...
    fun `recoverData - Check return true if function execute successfully`() =
        runTest {
            whenever(
                getDocument(Option.EDIT, FlowNote.INVOLVED, 1, 0)
            ).thenReturn(Result.success("123"))
            whenever(
                getName(Option.EDIT, FlowNote.INVOLVED, 1, 0)
            ).thenReturn(Result.success("TEST NAME"))
            whenever(
                getState(Option.EDIT, FlowNote.INVOLVED, 1, 0)
            ).thenReturn(Result.success(State.DEAD))
            whenever(
                getPhone(Option.EDIT, FlowNote.INVOLVED, 1, 0)
            ).thenReturn(Result.success("16999999999"))
            whenever(
                getAddress(FlowNote.INVOLVED, 1, 0)
            ).thenReturn(Result.success("ADDR"))
            whenever(
                getDetail(Option.EDIT, FlowNote.INVOLVED, 1, 0)
