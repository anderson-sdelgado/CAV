...
class IGetDocumentTest {
    ...
    @Test
    fun `Check return failure if have error in CardRepository getDocument - Option INSERT`() =
        runTest {
            whenever(
                cardRepository.getDocument()
            ).thenReturn(
                resultFailure(
                    "ICardRepository.getDocument",
                    "-",
                    Exception()
                )
            )
            ...
            assertEquals(
                ...
                "IGetDocument -> ICardRepository.getDocument"
            )
            assertEquals(
    ...
    fun `Check return failure if have error in CardRepository getDocumentPassengerInvolved - FlowNote PASSENGER_INVOLVED`() =
        runTest {
            whenever(
                cardRepository.getDocumentPassengerExternal(1, 2)
            ).thenReturn(
                resultFailure(
