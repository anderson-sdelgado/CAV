...
class IListWitnessColabTest {

    private val cardRepository = mock<CardRepository>()
    private val colabRepository = mock<ColabRepository>()
    private val usecase = IListWitnessColab(
        cardRepository = cardRepository,
        colabRepository = colabRepository
    )

    @Test
    fun `Check return failure if have error in CardRepository listWitnessColab`() =
        runTest {
            whenever(cardRepository.listWitnessColab()).thenReturn(resultFailure("CardRepository.listWitnessColab", Exception()))
            val result = usecase()
            assertEquals(true, result.isFailure)
            assertEquals("IListWitnessColab -> CardRepository.listWitnessColab", result.exceptionOrNull()!!.message)
        }
    ...
    fun `Check return failure if have error in ColabRepository listColabByRegList`() =
        runTest {
            ...
            whenever(cardRepository.listWitnessColab()).thenReturn(Result.success(list))
            ...
            val result = usecase()
            assertEquals(true, result.isFailure)
            assertEquals("IListWitnessColab -> ColabRepository.listColabByRegList", result.exceptionOrNull()!!.message)
        }
